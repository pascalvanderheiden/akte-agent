...
from proven query access. GitHub Code Access setup registers and
...
`sre_agent_name`, `sre_connect_telemetry`, `sre_connect_github`, `sre_github_repository_url`, and
`sre_github_branch`. Configuration inputs apply only when `provision` is selected;
both integration switches default to `true`. Blank overrides clear earlier values.
...
For core-only provisioning, set both integration switches to `false`;
...
The dedicated GitHub setup step also reruns enabled telemetry using the
environment's persisted options, including after deploy-only refresh; refresh
and application deployment themselves do not configure connectors. Disable telemetry with
...
Telemetry does not require that
