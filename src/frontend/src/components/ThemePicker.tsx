      ...
      {open && pos && typeof document !== "undefined" && createPortal(
        ...
        >
          <div className="px-3 py-2.5 border-b border-border-soft flex items-center justify-between">
            <div className="text-[11px] font-semibold uppercase tracking-wider text-muted">{t("theme.title")}</div>
            <button
              onClick={toggleMode}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-medium text-text truncate">{item.label}</div>
                    <div className="text-[10px] text-muted truncate">{t(`theme.${item.id}`)}</div>
                  </div>
                  {active && (
